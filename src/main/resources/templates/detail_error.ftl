...
<html xmlns:th="http://www.thymeleaf.org">
	...
	<body>
				...
				<div class="box box-primary">
		            <div class="box-body box-profile">
		              <ul class="list-group list-group-unbordered">
		                <li class="list-group-item">
		                  ...
		                  <span class="pull-right" style="color:#428bca;">
			                  	<#if index.knowledgePoint??>
									<#list index.knowledgePoint as point>
								 		${point.name};
									</#list>
								</#if>
		                  </span>
		                ...
		                <li class="list-group-item">
		                  <b>题型：</b>
		                   <span class="pull-right" style="color:#428bca;">${index.questionType.name}[ 共 ${size} 题 ]</span>
		                </li>
		              </ul>
				...
				<div class="box box-primary">
		            <div class="box-body box-profile">
						...
						<div class="form-group">
							<div class="col-md-4">
								<#if pre.id??>
									<a href="/detail_error_order?id=${pre.id}" class="btn btn-block btn-success">上一题</a>
								<#else>
							...
							<div class="col-md-4">
								<#if after.id??>
									<a href="/detail_error_order?id=${after.id}" class="btn btn-block btn-success">下一题</a>
								<#else>
									<a href="#" class="btn btn-block btn-success">没有下一题</a>
								</#if>
							</div>
							<div class="col-md-4">
								<a  class="btn btn-block btn-info" onclick="submit_answer(${index.questionType.id},${index.id},${after.id!0})">提交</a>
							</div>
						</div>
					</div>
				</div>
		...
		<script type="text/javascript">
			...
			function submit_answer(tId,qId,afterId){
				var answer;
				if(tId==1||tId==3){
					...
					success:function(msg){
						if(msg.code == 0){
							if(afterId>0){
								...
								window.location.href = "/detail_error_order?"+"id="+afterId;
							}else{
								layer.msg('回答正确！', {icon:6,time:1000});
								window.location.href = "/detail_error";
							}
						}else{
